...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold mb-8">Modifier le produit</h1>
            ...
            <div class="card">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title" id="title" class="input-field w-full @error('title') border-red-500 @enderror" value="{{ old('title', $product->title) }}" required>
                        @error('title')
                    ...
                    <div>
                        ...
                        <input type="number" name="quantity" id="quantity" min="0" class="input-field w-full @error('quantity') border-red-500 @enderror" value="{{ old('quantity', $product->quantity) }}" required>
                        @error('quantity')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->title }}" class="w-24 h-24 object-cover rounded">
                            <div class="flex-1">
                                <input type="file" name="image" id="image" accept="image/*" class="input-field w-full @error('image') border-red-500 @enderror">
                                <p class="text-sm text-gray-400 mt-1">Laissez vide pour conserver l'image actuelle. Format accepté : JPEG, PNG, JPG, GIF. Taille maximale : 2MB</p>
                            </div>
                        </div>
                        @error('image')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
